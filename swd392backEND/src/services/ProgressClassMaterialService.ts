...
class ProgressClassMaterialService {
  ...
  async getProgressByMaterial(classMaterialId: string) {
    return await ProgressClassMaterialRepo.getByClassMaterialId(
      classMaterialId,
    );
  }

  async getProgressByEnrollAndMaterial(
    enrollId: string,
    classMaterialId: string,
  ) {
    return await ProgressClassMaterialRepo.getByEnrollAndMaterial(
      enrollId,
      classMaterialId,
    );
  }
  ...
  async createProgress(enrollId: string, classMaterialId: string) {
    ...
    const existing = await ProgressClassMaterialRepo.getByEnrollAndMaterial(
      enrollId,
      classMaterialId,
    );
    if (existing) {
  ...
  async createProgressByClassAndStudent(
    classId: string,
    studentId: string,
    classMaterialId: string,
  ) {
    const enroll = await EnrollRepo.findEnrollByUserAndClass(
      studentId,
      classId,
    );
    if (!enroll) {
  ...
  async markAsCompleted(enrollId: string, classMaterialId: string) {
    const progress = await ProgressClassMaterialRepo.getByEnrollAndMaterial(
      enrollId,
      classMaterialId,
    );
    if (!progress) {
    ...
    return await ProgressClassMaterialRepo.updateCompletionStatus(
      progress._id.toString(),
      "completed",
    );
  }

  async markAsCompletedByClassAndStudent(
    classId: string,
    studentId: string,
    classMaterialId: string,
  ) {
  ...
  async getProgressByClassAndStudent(classId: string, studentId: string) {
    ...
    const totalMaterials =
      await ClassMaterialRepo.getActiveClassMaterialCount(classId);
    const progressRecords =
      await ProgressClassMaterialRepo.getByEnrollId(enrollId);
    const completed = progressRecords.filter(
      (p: any) => p.completion_status === "completed",
    ).length;
  ...
  async getClassProgressForAllStudents(classId: string) {
    ...
    const studentsProgress = await Promise.all(
      enrollments.map(async (enroll: any) => {
        const progressRecords = await ProgressClassMaterialRepo.getByEnrollId(
          enroll._id.toString(),
        );
        const completed = progressRecords.filter(
          (p: any) => p.completion_status === "completed",
  ...
  async bulkCreateForEnrollment(enrollId: string, classId: string) {
    const activeMaterials =
      await ClassMaterialRepo.getActiveClassMaterialsByClass(classId);
    await Promise.all(
